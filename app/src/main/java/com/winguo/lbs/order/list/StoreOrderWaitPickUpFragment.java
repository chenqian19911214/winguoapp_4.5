...
public class StoreOrderWaitPickUpFragment extends BaseFragment2 implements View.OnClickListener, BaseQuickAdapter.RequestLoadMoreListener, StoreOrderAdapter.IBaseViewHolderListener, BaseQuickAdapter.OnItemClickListener {
    ...
    private void requestData() {
        if (NetworkUtils.isConnectNet(mContext)) {
            ...
            StoreOrderControl.requestStoreOrder(mContext,mId, 3, 1, new StoreOrderControl.IStoreOrder() {
                @Override
    ...
    public void callback(StoreOrderBean.ContentBean item) {
        final String code = item.getMaker().getT_juchu_pick_code();
