...
public class PullUpToLoadMoreView extends ViewGroup {
    ...
    private void init() {
        ...
        post(new Runnable() {
            @Override
            public void run() {
                topScrollView = (TopScrollView) getChildAt(0);
                bottomScrollView = (BottomScrollView) getChildAt(1);
                topScrollView.setScrollListener(new TopScrollView.ScrollListener() {
                    @Override
                ...
                bottomScrollView.setScrollListener(new BottomScrollView.ScrollListener() {
                    @Override
                    public void onScrollToBottom() {
                ...
                if (bottomScrollVIewIsInTop) {
                    ...
                    if (dy < 0 && currPosition == 1) {
                        if (Math.abs(dy) >= scaledTouchSlop) {
                            CommonUtil.printI("现在滑动到第二页顶部了+++","父view要拦截事件========");
                            isIntercept = true;
                            lastY=y;//===
                        }
                    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        lastY = y;
        CommonUtil.printI("现在滑动的位置是第几页+++++++++:",currPosition+"");
        return true;
    }
