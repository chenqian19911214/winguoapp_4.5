 ...
 * @desc 所有订单
 */
public class AllFragment extends BaseFragment2 {
    /**
     * 订单列表数据
     */
    ...
    private void getDataByNetStatus() {
        ...
        if (NetWorkUtil.isNetworkAvailable(mContext)) {
            OrderHandler.searchOrder(mContext, handle, WinguoAccountDataMgr.getUserName(mContext), mPage, StartApp.limit, "0", 0);
        } else {
    ...
    protected void handleMsg(Message msg) {
        switch (msg.what) {
            case RequestCodeConstant.REQUEST_SEARCH_ORDER:
                ...
                no_has_more.setText(getActivity().getString(R.string.no_has_more_text));
            ...
            for (int j = 0; j < orderDetailAllBeanList.size(); j++) {
                ...
                if (isObject) {
                    ...
                    if (number == no) {
                        ...
                        commonBean.content = item.getIcon().getContent();//chenqian
        ...
        public void onScrollStateChanged(AbsListView absListView, int state) {
            ...
            if (state == AbsListView.OnScrollListener.SCROLL_STATE_IDLE && mIsEnd && mHasMore && !isLoadingMore) {
                if (NetWorkUtil.isNetworkAvailable(mContext)) {
                    ...
                    OrderHandler.searchOrder(handle, mPage, StartApp.limit, "0");
                } else {
                    ll_has_more.setVisibility(View.GONE);
                    no_has_more.setVisibility(View.VISIBLE);
                    no_has_more.setText(getString(R.string.request_net2));
    ...
    private class MyBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals("reselectedListener")) {
                if (intent.getIntExtra(ARG_POSITION, -1) == 0) {
                    if (NetWorkUtil.isNetworkAvailable(mContext)) {
                        isRefresh = true;
                        LoadDialog.show(mContext);
                        resetData();
