 ...
 * @desc ${TODD}
 */
public class OrderFragment extends BaseFragment2 {
    /**
     * 订单列表数据
     */
    ...
    private FrameLayout order_list_container;
    private HashMap<String, ViewPagerBean> mViewPagerBeans;
    private ViewPagerBean viewPagerBean;

    public static OrderFragment newInstance(int position, String orderStatus) {
    ...
    protected void initView(View view) {
        order_list_container = (FrameLayout) view.findViewById(R.id.order_list_container);
         /*---------------------------------------------*/
        //(给容器添加布局)
        ...
        if (mViewPagerBeans == null) {
            mViewPagerBeans = new HashMap<>();
        }
    }
    ...
    protected void loadData() {
        ...
        //不存在
        if (!mViewPagerBeans.containsKey(mOrderStatus)) {
            //加载数据
            viewPagerBean = new ViewPagerBean();
            LoadDialog.show(mContext);
            getDataByNetStatus();
        } else {
            viewPagerBean = mViewPagerBeans.get(mOrderStatus);
            //存在
    ...
    private void getDataByNetStatus() {
        ...
        if (NetWorkUtil.isNetworkAvailable(mContext)) {
            OrderHandler.searchOrder(mContext, handle, WinguoAccountDataMgr.getUserName(mContext), mPage, StartApp.limit, mOrderStatus, 0);
        } else {
    ...
    private void setHasMore(int orderAll) {
        ...
        if (orderAll != 0) {
            ...
            mViewPagerBeans.put(mOrderStatus, viewPagerBean);
        }
    }
        ...
        public void onScrollStateChanged(AbsListView absListView, int state) {
            ...
            if (state == AbsListView.OnScrollListener.SCROLL_STATE_IDLE && mIsEnd && mHasMore && !isLoadingMore) {
                if (NetWorkUtil.isNetworkAvailable(mContext)) {
                    ...
                    //TODO
                    LoadDialog.show(mContext);
                    OrderHandler.searchOrder(handle,mPage, StartApp.limit, mOrderStatus);
                } else {
                    ll_has_more.setVisibility(View.GONE);
                    no_has_more.setVisibility(View.VISIBLE);
                    no_has_more.setText(getString(R.string.request_net2));
...
/*----------------------Adapter部分-----------------------*/
    ...
    private class OrderAdapter extends BaseAdapter {
        ...
        private void adapterInitData(ViewHolder viewHolder, final OrderDataBean orderDataBean) {
            ...
            GlideUtil.getInstance().loadImage(mContext, orderDataBean.getImage().getContent(), R.drawable.icon, R.drawable.error_img, viewHolder.order_goods_picture_iv);
            //单价
            ...
            //规格和颜色
            viewHolder.order_goods_color_tv.setText(orderDataBean.getColor_alias() + ": ");
            viewHolder.order_goods_color2_tv.setText(orderDataBean.getColor_name().equals("") ? "无" : orderDataBean.getColor_name());
            viewHolder.order_goods_standard_tv.setText(orderDataBean.getSize_alias() + ": ");
            viewHolder.order_goods_standard2_tv.setText(orderDataBean.getSku_size().equals("") ? "无" : orderDataBean.getSku_size());
            //订单状态判断
            switch (order_status) {
                case "待付款":
                    viewHolder.order_left_btn.setVisibility(View.INVISIBLE);
                    viewHolder.order_right_btn.setVisibility(View.VISIBLE);
                    viewHolder.order_state_tv.setText(R.string.order_wait_pay);
                    viewHolder.order_right_btn.setText(R.string.order_pay);
                    //立即支付
                    viewHolder.order_right_btn.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View view) {
                            //传给支付界面:
                            //1.地址
                            ItemBean address = orderDataBean.getAddress();
                            //2.商品信息
                            ArrayList<CommonBean> commonBeans = orderDataBean.getCommonBeans();
                            //3.订单id
                            int oid = orderDataBean.getOid();
                            //跳转
                            Intent intent = new Intent(mContext, ConfirmOrderActivity.class);
                            intent.putExtra("commonBeen", commonBeans);
                            intent.putExtra("mItemBean", address);
                            intent.putExtra("orderId", String.valueOf(oid));
                            startActivity(intent);
                        }
                    });
                    break;
                case "待出货":
                    ...
                    break;
                case "待收货":
                    viewHolder.order_state_tv.setText(R.string.order_wait_receive);
                    viewHolder.order_left_btn.setVisibility(View.VISIBLE);
                    viewHolder.order_right_btn.setVisibility(View.VISIBLE);
                    viewHolder.order_left_btn.setText(R.string.order_confirm_receive);
                    viewHolder.order_right_btn.setText(R.string.order_look_delivery);
                    viewHolder.order_right_btn.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View view) {
                            Intent intent = new Intent(mContext, DeliveryActivity.class);
                            intent.putExtra(ActionUtil.ACTION_DELIVERY, orderDataBean.getDeliveryRootBean());
                            startActivity(intent);
                        }
                    });
                    viewHolder.order_left_btn.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            PayPwdPopuWindow payPwdPopuWindow = new PayPwdPopuWindow(mActivity, new PayPwdPopuWindow.IPopuWindowConfirmListener() {
                                @Override
                                public void confirm(String pwd) {
                                    LoadDialog.show(mContext);
                                    OrderHandler.ConfirmReceive(mActivity, handle, orderDataBean.getOid(), pwd);
                                }

                                @Override
                                public void forgetPwd() {
                                    //TODO
                                    Intent it = new Intent(mActivity, ModifyPayWayActivity.class);
                                    startActivity(it);
                            ...
                            payPwdPopuWindow.showAtLocation(order_list_container, Gravity.BOTTOM, 0, 0);
                        }
                    });
                    break;
                case "已取消":
                    viewHolder.order_state_tv.setText(R.string.order_wait_close);
                    viewHolder.order_left_btn.setVisibility(View.INVISIBLE);
                    viewHolder.order_right_btn.setVisibility(View.VISIBLE);
                    viewHolder.order_right_btn.setText(R.string.order_pay_chongxin_);
                    viewHolder.order_right_btn.setOnClickListener(new View.OnClickListener() {
                        @Override
                        public void onClick(View v) {
                            Intent intent = new Intent();
                            intent.putExtra("gid", Integer.valueOf(orderDataBean.getGid()));
                    ...
                    break;
                case "已完成":
                    viewHolder.order_state_tv.setText(R.string.order_wait_finish);
                    viewHolder.order_left_btn.setVisibility(View.GONE);
                    viewHolder.order_right_btn.setVisibility(View.GONE);
                    //viewHolder.order_left_btn.setText(R.string.order_look_comment);
                    viewHolder.order_right_btn.setText(R.string.order_pay_zailai);
                    break;
                case "待评价":
                    viewHolder.order_state_tv.setText(R.string.order_wait_finish);
                    viewHolder.order_left_btn.setVisibility(View.GONE);
                    viewHolder.order_right_btn.setVisibility(View.VISIBLE);
                    //viewHolder.order_left_btn.setText(R.string.order_pay_zaici);
                    viewHolder.order_right_btn.setOnClickListener(new View.OnClickListener() {
                    ...
                    viewHolder.order_right_btn.setText(R.string.order_pay_zaici);
                    break;
