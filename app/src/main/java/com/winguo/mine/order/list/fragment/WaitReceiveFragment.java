 ...
 * @desc 待收货
 */
public class WaitReceiveFragment extends BaseFragment2 {
    /**
     * 订单列表数据
     */
    ...
    public static WaitReceiveFragment newInstance(int position) {
        WaitReceiveFragment f = new WaitReceiveFragment();
        Bundle b = new Bundle();
    ...
    private void getDataByNetStatus() {
        ...
        if (NetWorkUtil.isNetworkAvailable(mContext)) {
            OrderHandler.searchOrder(mContext, handle, WinguoAccountDataMgr.getUserName(mContext), mPage, StartApp.limit, "3", 0);
        } else {
        ...
        public void onScrollStateChanged(AbsListView absListView, int state) {
            ...
            if (state == AbsListView.OnScrollListener.SCROLL_STATE_IDLE && mIsEnd && mHasMore && !isLoadingMore) {
                if (NetWorkUtil.isNetworkAvailable(mContext)) {
                    ...
                    OrderHandler.searchOrder(handle,mPage, StartApp.limit, "3");
                } else {
                    ll_has_more.setVisibility(View.GONE);
                    no_has_more.setVisibility(View.VISIBLE);
                    no_has_more.setText(getString(R.string.request_net2));
    ...
    private class MyBroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals("reselectedListener")) {
                if (intent.getIntExtra(ARG_POSITION, -1) == 3) {
                    if (NetWorkUtil.isNetworkAvailable(mContext)) {
                        isRefresh = true;
                        LoadDialog.show(mContext);
                        resetData();
